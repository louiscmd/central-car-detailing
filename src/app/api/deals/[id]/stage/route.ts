...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const lead = await prisma.lead.findFirst({ where: { id, userId: session.user.id }, include: { stage: true } });
  ...
  await prisma.lead.update({ where: { id }, data: { stageId } });
  ...
  if (auto) {
    ...
    await prisma.followUp.create({
      data: { leadId: id, dueDate, note: auto.note, priority: auto.priority as never },
    });
    await prisma.leadActivity.create({
      data: { leadId: id, type: "FOLLOW_UP_SCHEDULED", note: `Auto-scheduled: ${auto.note}` },
    });
  }
